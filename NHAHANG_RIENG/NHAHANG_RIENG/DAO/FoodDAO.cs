...
namespace NHAHANG_RIENG.DAO
{
    public class FoodDAO
    {
        ...
        {
            List<Food> list = new List<Food>();
             string query = String.Format("SELECT a.ID ,a.NAME as 'Name' , B.NAME as 'Catery', A.GIA as 'Gia' FROM DBO.FOOD a, DBO.FOODCATERY B  where a.IDCATERY=b.ID and DBO.fuConvertToUnsign1(A.NAME)  Like N'%' + DBO.fuConvertToUnsign1(N'{0}') + '%'",name);
           // string query = String.Format("fuConvertToUnsign1 @strInput", name);
            DataTable data = DataProvider.Instance.ExcuteQuery(query);
